...
rule find_intersecting_snps:
	...
	input:
		...
		snps = ["analysis/wasp/variants/{sample}/" + "%s.snps.txt.gz" % s for s in get_chr()]
	output:
		fastq1 = "analysis/wasp/find_intersecting_snps/{sample}/{sample}.remap.fq.gz",
	...
	shell:
		...
		"    --is_sorted --output_dir analysis/wasp/find_intersecting_snps/{wildcards.sample} --snp_dir analysis/wasp/variants/{wildcards.sample} {input.bam} > {output.log}"
...
rule remap_at_variants:
	...
	shell:
#		"bowtie2 -x {config[bowtie2_index]} -U {input.fastq1}"
		"bwa mem {config[bwa_index]} {input.fastq1}"
