# This script is adapted from https://github.com/gusevlab/fusion_twas with minor modifications as noted

#MODIFICATION: This adds the R library in the fusion conda env,
#which cannot currently be created correctly by snakemake because
#plink2R is not available from conda. (so downloaded plink2R
#into the indicated directory).
.libPaths( c( .libPaths(), "~/.conda/envs/fusion/lib/R/library") )
...
# MODIFICATION: added to deal with cases where there are no peaks to be displayed on this chr (otherwise throws an error later). Close bracket is at the end of the script
if(nrow(wgtlist)>0) {
	...
	for ( g in dup.genes ) {
		...
		for ( i in 1:length( cur.keep ) ) {
			...
			if ( !is.na(opt$omnibus_corr) && opt$omnibus_corr != "best" ) {
		        ...
		        if ( substr( (colnames(wgt.matrix))[ mod ],1,4) == "top1" ) wgt.matrix[ -which.max(wgt.matrix[,mod]^2)  , mod] = 0
...
for ( i in 1:nrow(wgtlist) ) {
	...
	# models with "top1" in the name get restricted to the top SNP
	if ( substr( (colnames(wgt.matrix))[ mod ],1,4) == "top1" ) wgt.matrix[ -which.max(wgt.matrix[,mod]^2)  , mod] = 0
...
for ( i in 1:length(cons.loc.starts) ) {
	...
	if ( sum(cond.z^2 > zthresh^2) == 0 ) {
		...
		while ( sum(cond.z^2 > zthresh^2) != 0 ) {
			# add most conditionally significant feature 
			joint.keep[ which.max(cond.z^2) ] = T #add feature to joint model
			...
			cur.dinv = solve(ge_g.ld[joint.keep,joint.keep]) 
			# iterate through peaks not in the joint model
			for ( ii in which(cond.z != 0) ) { 
				#if added to joint model or r2 too high, remove for next iteration
				if ( joint.keep[ii] ) {
					cond.z[ii] = 0
				} else if ( max(ge_g.ld[ii,joint.keep]^2) > opt$max_r2 ) {
					cond.z[ii] = 0
				} else {
					# estimate conditional effect size, conditioing each remaining individual peak on the joint model
		...
		if ( opt$plot ) {
			...
			ones=ifelse(tot==1,1,ceiling(log(tot)/3))  #NEW
			lay.eqtl = matrix( c( rep(1,ones),2,2,3,3) , ncol=1 )
			lay.gwas = matrix( c( rep(1,ones),2,2,2) , ncol=1 )
...
} #MODIFICATION: end of large if statement containing most of the script. Added to deal with cases where there are no peaks to be displayed on this chr
