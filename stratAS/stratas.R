...
option_list = list(
	...
	make_option("--weights_out", action="store", default="WEIGHTS" , type='character',
		help="Directory to place files with model SNP weights") #added 1/29/21 by SCB
)
opt = parse_args(OptionParser(option_list=option_list))

###tmp for testing
#opt$input = "analysis/stratasrun/split/counts.02"
#opt$samples = "samples.txt"
#opt$peaks = "data/peaks/consensus/AR.consensus.stratas.bed"
#opt$global_param = "analysis/stratasrun/all.global.params"
#opt$local_param = "analysis/stratasrun/all.local.params"
#opt$max_rho = 0.2
#opt$min_cov = 1
#opt$window = 10000
#opt$predict_snps = "LDREF/hm3.pos"
#opt$total_matrix = "analysis/predict/total.mat"
#opt$covar = "analysis/predict/covar.mat"
#opt$predict_only = TRUE
#opt$predict = TRUE
#opt$weights_out = "analysis/predict/WEIGHTS"
...
if ( opt$predict ) {
	...
	#x.tot = matrix of [individuals x snps] with total non-ref alleles (0 to 2)
	#x.hap = matrix of [individuals x snps] haplotype specific alleles (hap1 - hap2; -1 to 1)	
	#y.tot = activity at this peak for each individual
	#y.h1 = haplotype 1 reads for this peak
	#y.h2 = haplotype 2 reads for this peak
	#output = peak name
		...
		tot.cut = floor(seq(0,length(tot.ord),length.out=(folds+1))) # define cuts between folds
		...
		x.tot.scaled = scale(x.tot) #center to 0 and divide by standard deviation
		...
		for ( i in 2:(folds+1) ) {
			...
			tot.heldout = tot.ord[ batch ] #test samples
			c.tot = pred.lasso( x = x.tot[-tot.heldout,] , y = y.tot.scaled[-tot.heldout] ) #get coefficients for cQTL prediction with lasso
			tot.pred[ tot.heldout , 1 ] = x.tot[tot.heldout,] %*% c.tot #predict heldout (test) samples 
			c.tot.top = pred.marginal( x = x.tot.scaled[-tot.heldout,] , y = y.tot.scaled[-tot.heldout] , top=FALSE ) #get marginal effects for each snps for predicting total activity (used as coefficients for prediction)  
			if ( sum(hap.keep) >= 10 ) {
				batch = (hap.cut[i-1]+1):hap.cut[i]
				hap.heldout = hap.ord[ batch ]
				
				c.hap = pred.lasso( x = x.hap[-hap.heldout,] , y = y.hap[-hap.heldout] , w = sqrt(hap.wgt[-hap.heldout]) ) #get coefficients for allele-specific predicitons with lasso
				c.hap.top = pred.marginal( x = x.hap.scaled[-hap.heldout,] , y = y.hap.scaled[-hap.heldout] , top=FALSE ) #get marginal effects for each snp for predicting allele-specific activity (used as the coefficient for prediction)

				c.combined = pred.combined( x1 = x.tot.scaled[-tot.heldout,] , x2 = x.hap.scaled[-hap.heldout,] , y1 = y.tot.scaled[-tot.heldout] , y2 = y.hap.scaled[-hap.heldout] ) #get coefficients for combined cQTL and allele-specific model
				...
				# get the max QTL
				c.combined.top[ - which.max( c.combined.top^2 ) ] = 0 #take the largest cQTL coefficient
				c.hap.top[ - which.max( c.hap.top^2 ) ] = 0 #take the largest allele specific coefficient
				
				tot.pred[ tot.heldout , 2 ] = (x.tot[tot.heldout,] - 1) %*% c.hap	#lasso allele specific predictions for cQTLs
				tot.pred[ tot.heldout , 3 ] = x.tot.scaled[tot.heldout,] %*% c.combined	#combined total activity predictions for cQTLs
				tot.pred[ tot.heldout , 4 ] = x.tot.scaled[tot.heldout,] %*% c.hap.top	#total activity predictions for 
...
#		save( wgt.matrix , snps , cv.performance , hsq, hsq.pv , hsq.as , hsq.qtl ,  N.tot , N.as , N.qt , file = paste( "WEIGHTS/" , output , ".wgt.RDat" , sep='' ) )
		save( wgt.matrix , snps , cv.performance , hsq, hsq.pv , hsq.as , hsq.qtl ,  N.tot , N.as , N.qt , file = paste(opt$weights_out, "/", output , ".wgt.RDat" , sep='' ) )
