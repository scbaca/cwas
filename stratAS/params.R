	...
	if ( !is.na(opt$group) ) {
		...
		for ( i in 1:(opt$group) ) {
			...
			if ( cur.tot > opt$min_snps ) {
				# Added the following tryCatch statement to deal with vglm errors - SCB 2/12/20:
				tryCatch({
				...
				{cat(err$message,"\nWARNING: skipping the following cnv segment due to a vglm error:", 
				  paste0("chr", cnv$CHR[c], ":", cnv$P0[c], "-", cnv$P1[c]))})
